            ...
            <header id="banner-header" className="flex flex-col gap-2 sm:gap-3 w-full shrink-0">
              <div className="flex items-center justify-between gap-4 w-full">
                ...
                <div className="flex items-center gap-2 sm:gap-6 md:gap-10">
                  ...
                  <nav className="hidden md:flex items-center gap-6 md:gap-8 text-sm md:text-base font-medium text-neutral-300">
                    {content.navLinks.map((link, idx) => (
                      <span
                        key={idx}
                        onClick={() => handleNavClick(link)}
                        className={`hover:text-white transition-colors cursor-pointer tracking-wide ${link.toLowerCase() === 'packages' ? 'text-[#d4af37] font-bold' : ''
                          }`}
                      >
                        {link}
                      </span>
